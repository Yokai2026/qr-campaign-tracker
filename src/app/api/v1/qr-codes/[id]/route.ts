...
const UPDATEABLE = [
  'target_url', 'active', 'valid_from', 'valid_until', 'note', 'title',
  'utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_id',
  'qr_fg_color', 'qr_bg_color', 'max_scans', 'short_host', 'placement_id',
] as const;
...
export async function GET(req: NextRequest, { params }: Ctx) {
  ...
  if (!data) return apiError(404, 'QR-Code not found');
  return apiOk({ data });
}
...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  const sb = serviceRoleClient();
  const { data, error } = await sb
    .from('qr_codes').update(update).eq('id', id).eq('created_by', auth.userId)
    .select('*').maybeSingle();
...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  ...
  const { error, count } = await sb
    .from('qr_codes').delete({ count: 'exact' }).eq('id', id).eq('created_by', auth.userId);
  if (error) return apiError(400, error.message);
  if (!count) return apiError(404, 'QR-Code not found');
